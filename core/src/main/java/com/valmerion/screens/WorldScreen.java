...
public class WorldScreen extends BaseScreen {
    ...
    public WorldScreen(ValmerionGame game) {
        ...
        npcPlaceholder = PlaceholderTextures.solid(80, 96,
        ...
        hpBar = new HealthBar(20, Constants.WORLD_HEIGHT - 54, 220, 30, "HP");
    }
    ...
    private void update(float delta) {
        ...
        if (stage == 0) {
            if (Math.abs(player.getPosition().x - ZAK_X) < NPC_INTERACT_DIST)
                nearNpc = "zak";
        } else if (stage == 1) {
            if (Math.abs(player.getPosition().x - TENKAI_X) < NPC_INTERACT_DIST)
                nearNpc = "tenkai";
        } else if (stage >= 2) {
    ...
    private void startDialogue(String npc) {
        ...
        if ("zak".equals(npc)) {
            dialogueOverlay.show(buildZakDialogue());
        } else if ("tenkai".equals(npc)) {
            dialogueOverlay.show(buildTenkaiDialogue());
        }
    }
    ...
    private DialogueLine[] buildZakDialogue() {
        return new DialogueLine[]{
            ...
            new DialogueLine("Зак",
                "Хазан — старый оружейник. Он проверит, какое оружие тебе ближе. Потом — Чёрная Дверь. Докажи, что умеешь сражаться."),
        };
    }
    ...
    private DialogueLine[] buildTenkaiDialogue() {
        return new DialogueLine[]{
            new DialogueLine("Тенкай",
                "Зак рассказал о тебе. Говорит, ты прошёл Чёрную Дверь используя все четыре класса. Это редкий дар."),
            new DialogueLine("Тенкай",
